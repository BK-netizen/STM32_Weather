...
void OLED_ClearRect(unsigned char x0, unsigned char y0, unsigned char x1, unsigned char y1)
{
    unsigned char x, y;
    unsigned char startPage, endPage;

    // 计算起始和结束页数，确保 y 坐标在页范围内
    startPage = y0 / 8;
    endPage = (y1 - 1) / 8;  // (y1-1) / 8 保证 y1 被正确映射到页面

    // 清除矩形区域
    for (y = startPage; y <= endPage; y++) {
        for (x = x0; x < x1; x++) {
            OLED_SetPos(x, y);
            WriteDat(0x00);  // 清除区域，设置为 0x00 关闭显示
        }
    }
}

void OLED_DrawRect(unsigned char x0, unsigned char y0, unsigned char x1, unsigned char y1)
{
    unsigned char x, y;
    unsigned char startPage, endPage;
    ...
    // 计算起始和结束页数
    startPage = y0 / 8;
    endPage = (y1 - 1) / 8;  // (y1-1) / 8 确保 y1 在正确的页内

    // 绘制上边界
    for (x = x0; x < x1; x++) {
        OLED_SetPos(x, startPage);  // 上边界的 y 坐标
        WriteDat(0xFF);  // 点亮像素
    }

    // 绘制下边界
    for (x = x0; x < x1; x++) {
        OLED_SetPos(x, endPage);  // 下边界的 y 坐标
        WriteDat(0xFF);  // 点亮像素
    }

    // 绘制左边界
    for (y = startPage; y <= endPage; y++) {
        OLED_SetPos(x0, y);  // 左边界的 x 坐标
        WriteDat(0xFF);  // 点亮像素
    }

    // 绘制右边界
    for (y = startPage; y <= endPage; y++) {
        OLED_SetPos(x1, y);  // 右边界的 x 坐标
        WriteDat(0xFF);  // 点亮像素
    }
}

// 绘制水平直线
void OLED_DrawHorizontalLine(unsigned char x0, unsigned char y, unsigned char x1)
{
    unsigned char x;
    if (x0 > x1) {
    ...
    // 遍历 x0 到 x1 的所有像素，逐个点亮
    for (x = x0; x <= x1; x++) {
        OLED_SetPos(x, y);  // 设置位置到指定的坐标
        WriteDat(0x01);     // 点亮该位置的像素
    }
}


//uint32_t countdown_time = 1800;
//uint32_t minutes;
//uint32_t seconds;
...
//	
//    OLED_ShowNum(32,0,minutes, 2,16);  // 显示分钟
//	OLED_ShowCN(48,0,10); 
//    OLED_ShowNum(64,0, seconds, 2,16);  // 显示秒数
//    OLED_ShowCN(80,0,11); 
